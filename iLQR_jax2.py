...
def model_func_risan(x, u, dt):
    cos_ = jnp.cos(x[2])
    sin_ = jnp.sin(x[2])
    dx = jnp.array([cos_ * (x[3]+u[0]),
                    sin_ * (x[3]+u[0]),
                    x[4]+u[1]])
    x_next1 = x[:3] + dx * dt
    x_next2 = x[3:5] + u
    x_next = jnp.concatenate([x_next1,x_next2],0)
    return x_next
...
class Cont_Args:
    ...
    R_del = 0 * jnp.eye(2, dtype=jnp.float32)
...
def iLQR_control(x,us):
    ...
    def Forward(xs,us,Ks,ds,dV1,dV2,J):
        ...
        def ls_check(val):
            ...
            return jnp.logical_and((jnp.logical_or(1e-4 > z, z > 10)), ls_iteration < 20)
