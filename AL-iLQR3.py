...
@jax.jit
def const_x_func(x, ev_pos, margin=1.0):
    def quadratic(d):
        return jnp.where(jnp.abs(d) < 1, 1 - d**2, 0.0)
    distance = jnp.linalg.norm(x-ev_pos, ord=2)
    outlier = (distance - args.d) / margin
    value = jnp.where(outlier <= args.d, 1.0, quadratic(outlier))
    return value
...
def stage_cost(x,u,k):
    cost = 0.5 * ( jnp.sqrt((x-args.x_ob) @ args.Q @ (x-args.x_ob)) \
...
def AL_iLEQG(x,xs,us):
    ...
    args.mus = 100*jnp.ones((args.N, con_dim),dtype=jnp.float32)
    ...
    for i in range(30):
        ...
        args.lambdas = update_lambda_mu(xs)
        args.lambdas_u = update_lambda_mu_u(us)
        args.mus = jnp.minimum(1e10,args.mus*100)
        args.mus_u = jnp.minimum(1e10,args.mus_u*100)
